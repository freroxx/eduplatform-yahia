...
import { BookOpen, FileText, Video, ArrowLeft, Calendar, Clock, Target, Maximize2, PenTool } from "lucide-react";
...
const Lessons = () => {
  ...
  const getCurrentLessons = () => {
    ...
    } else if (selectedSubject.id === 'svt') {
      return { semester1: svtLessons, semester2: svtSecondSemester };
    } else {
      return { semester1: frenchLessons, semester2: frenchSecondSemester };
    }
  };
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center space-x-3 mb-3">
              <span className={`rounded-full w-10 h-10 flex items-center justify-center text-sm font-bold text-white ${selectedSubject.id === 'math' ? 'bg-indigo-600' : selectedSubject.id === 'physics' ? 'bg-emerald-600' : selectedSubject.id === 'svt' ? 'bg-green-600' : 'bg-purple-600'}`}>
                {lesson.id > 300 ? lesson.id - 300 : lesson.id > 200 ? lesson.id - 200 : lesson.id > 100 ? lesson.id - 100 : index + 1}
              </span>
      ...
      <CardContent>
        <div className="flex flex-wrap gap-3">
          <Link to={`/lesson/${lesson.id}/course`}>
            <Button variant="default" size="sm" className={`shadow-md hover:shadow-lg transition-all duration-200 ${selectedSubject.id === 'math' ? 'bg-indigo-600 hover:bg-indigo-700' : selectedSubject.id === 'physics' ? 'bg-emerald-600 hover:bg-emerald-700' : selectedSubject.id === 'svt' ? 'bg-green-600 hover:bg-green-700' : 'bg-purple-600 hover:bg-purple-700'}`}>
        ...
        >
          ...
          <div className="flex justify-center space-x-8 text-sm text-muted-foreground">
            ...
            <div className="flex items-center">
              ...
              <span>{selectedSubject.id === 'math' ? '12h30' : selectedSubject.id === 'french' ? '4h30' : '18h45'} de contenu</span>
            </div>
          </div>
        </motion.div>
